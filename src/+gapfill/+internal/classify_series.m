...
%GAPFILL.INTERNAL.CLASSIFY_SERIES Assign a coarse structural class to a series.
    ...
    scores.persistent = max(0, gapfill.internal.nan_to_zero(stats.persistence_lag1)) + ...
        ...
        max(0, gapfill.internal.nan_to_zero(stats.hurst_effective) - 0.5) * 2;
    scores.antipersistent = max(0, 0.5 - gapfill.internal.nan_to_zero(stats.hurst_effective)) * 2 + ...
        max(0, -gapfill.internal.nan_to_zero(stats.persistence_lag1));
    ...
    baseLabels = {'smooth', 'persistent', 'antipersistent', 'seasonal', 'bursty'};
    baseVector = [scores.smooth, scores.persistent, scores.antipersistent, scores.seasonal, scores.bursty];
    [baseWinner, idx] = max(baseVector);
    baseLabel = baseLabels{idx};

    hasRegimeChanges = gapfill.internal.nan_to_zero(stats.regime_change_score) > 0.20 || ...
        gapfill.internal.nan_to_zero(stats.regime_change_count) >= 2;
    isMultiscale = gapfill.internal.nan_to_zero(stats.regime_multiscale_score) > 0.06;
    isLongMemory = ~isnan(stats.hurst_effective) && abs(stats.hurst_effective - 0.5) >= 0.08;
    ...
    if hasRegimeChanges
        ...
        if ~strcmp(baseLabel, 'seasonal') && ...
                (gapfill.internal.nan_to_zero(stats.regime_change_score) > 0.55 || ...
                scores.regime_switching >= max(0.45, 0.85 * baseWinner) || ...
                gapfill.internal.nan_to_zero(stats.regime_change_count) >= 3)
